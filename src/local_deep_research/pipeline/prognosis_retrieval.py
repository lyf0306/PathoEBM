...
class PrognosisRetrieval:
    ...
    def __init__(self, tool_planning_model, fast_model, mcp_tool_client,
        ...
        self.treatment_context = treatment_context or ""
    ...
    async def _plan_next(self, base_info: str, accumulated: list,
        ...
        1. **第1步必须检索大样本生存率**：使用 NCDB/SEER 数据库，必须包含 (NCDB OR SEER OR "National Cancer Database")，分期必须降维为大分期，末尾加 AND survival AND 2018:2026[dp]。
           🚨 **【癌种以患者信息为准】**：**绝对禁止**将患者的癌种替换为其他癌种。患者信息中的诊断是什么癌种，检索词就必须用该癌种的 MeSH 术语（如子宫内膜癌 → "Uterine Neoplasms" 或 "Endometrial Neoplasms"），禁止使用 "Colorectal Neoplasms" 或其他不相关癌种。
        2. **后续步骤深入分子/亚组**：围绕分子分型（p53abn等）与预后的关联，不加 NCDB/SEER 限定，末尾加 AND (prognos* OR survival OR outcome*) AND 2018:2026[dp]。
           同样必须使用患者信息中的癌种，禁止替换为其他癌种。
        ...
        try:
            resp = await invoke_with_timeout_and_retry(
                self.fast_model, plan_prompt, timeout=300.0, max_retries=3
            )
    ...
    def _get_patient_info(self) -> str:
        ...
        if isinstance(oncology_core, dict):
            diag = oncology_core.get("diagnosis_and_stage", "") or ""
            patho = oncology_core.get("pathology_and_molecular", "") or ""
            info = f"{diag} {patho}".strip()
            if info:
                return info

        # Fallback: extract cancer type from treatment_context
        if self.treatment_context:
            ctx = self.treatment_context[:1000].lower()
            # Match known cancer patterns
            cancer_patterns = [
                r'(endometrial cancer|endometrial carcinoma|uterine cancer)',
                r'(子宫内膜癌|子宫内膜样癌|浆液性癌|子宫肉瘤|子宫肌瘤)',
            ]
            for pat in cancer_patterns:
                m = re.search(pat, ctx, re.IGNORECASE)
                if m:
                    return f"患者诊断为{m.group(1)}（从原始病历提取）"
            # Fallback within fallback: just note the context length
            return f"Endometrial cancer patient (from {len(self.treatment_context)}-char treatment context)"

        # Ultimate fallback
        return "Endometrial cancer patient"
