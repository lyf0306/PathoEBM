...
def safe_json_from_text(text: str) -> Optional[Dict]:
    ...
    # Phase 1: Remove properly fenced code blocks ```json ... ```
    cleaned = re.sub(r"```(?:json)?\s*([\s\S]*?)\s*```", r"\1", text, flags=re.I)

    # Phase 2: If no closing ``` exists, strip leading ```json or ``` line
    if cleaned == text and "```" in text:
        cleaned = re.sub(r"^```(?:json)?\s*\n?", "", text, flags=re.I)
        cleaned = re.sub(r"\n?\s*```\s*$", "", cleaned).strip()

    # Find the first '{' — the JSON must start there
    start = cleaned.find("{")
    if start == -1:
        return None

    # Method A: Walk forward tracking brace depth, try to parse each
    # complete top-level object. This handles trailing text after JSON.
    depth = 0
    ...
    # Method B: raw_decode handles valid JSON followed by stray text.
    try:
