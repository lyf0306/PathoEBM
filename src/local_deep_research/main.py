...
2. **【极度重要】并发症分级（Comorbidity Stratification）**：
   - **重大合并症 (major_comorbidities)**：提取对患者长期生存预后及基础代谢有重大影响的全身性疾病（如：糖尿病、肥胖、高血压、冠心病、自身免疫病等），供后续多学科随诊参考。
   - **次要异常 (incidental_findings)**：提取局灶性、轻度且暂不影响全身肿瘤系统治疗的异常（如：浅表胃炎、轻度脂肪肝、肺部散在慢性炎症、主动脉壁钙化等）。

3. **【新增：强排他性规则】主要肿瘤治疗方案 (main_oncology_treatment)**：提取草稿中**最终针对该患者实际拍板的个性化方案**。
   🚨 **【反缝合怪与个体化降级排他红线（极度重要！）】**：仔细阅读草稿中是否有因“严重合并症、心血管极高危、生理耐受差”导致的**方案降级、禁忌症警告或治疗取消**（例如明确写了“否决常规EBRT”、“大幅降级”、“姑息性或低毒性方案”）。
   ❌ 错误做法：将指南的“高强度标准方案”和针对患者的“降级方案”矛盾地缝合在一起（如输出：“建议盆腔外照射联合化疗。建议取消放疗”）。
   ✅ 正确做法：**直接抛弃（排他）**草稿中提及的通用指南高强度标准，**仅且只提取**妥协后的最终个体化降级方案。例如直接输出：“鉴于患者极高心血管风险，取消常规盆腔外照射(EBRT)及强效联合化疗，降级为单药卡铂或紫杉醇周疗等低强度/姑息方案。”

4. **循证问题 (clinical_questions_for_ebm)**：提取草稿中提出的 PICO 问题或检索需求。
   🚨 **【反数据丢失绝对红线（极度重要！）】**：你提取的问题不仅要包含 PICO 结构，还**必须原封不动地保留**草稿中提及的任何具体“临床试验代号”（如 PORTEC-3, GOG-0258, PORTEC-4a 等）、“特定数据库”以及“专属检索要求”！
   ❌ 错误做法：为了语句精简而把试验代号删掉。
   ✅ 正确做法：完整保留患者原话中的查证诉求，如“请重点检索 GOG-0258 临床试验的长期数据...”。
