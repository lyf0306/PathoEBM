...
class ReActSearchAgent:
        ...
        for round_idx in range(max_rounds):
            ...
            if not all_results and round_idx == 0:
                ...
                if relaxed and relaxed != query:
                    ...
                    continue
                # Relaxation failed or not possible — give up
                return ""
    ...
    async def _check_sufficiency(self, query: str, synthesis: str) -> dict:
        ...
        You are a clinical evidence auditor. Evaluate whether the synthesis below adequately answers the original query.

        【Original query】: {query}

        【⚠️ Fact-check — regex pre-scan result】: {fact_line}

        【Your task】:
        Read the full synthesis below. Judge whether it provides concrete numerical evidence
        (survival %, HR, P-value) relevant to the query. If the regex pre-scan already
        found data, do NOT claim it is missing unless you can point to a specific
        placeholder/template artifact (e.g. "[N=未报告]") that proves it's fake.

        【Rules for refined_query when insufficient】:
        - Must include statistical abbreviations: (OS OR PFS OR HR OR "hazard ratio")
        - For molecular gaps: (p53 OR TP53 OR POLE OR MMR OR MSI)
        - NEVER add comorbidity terms (hypertension, diabetes, etc.)
        - Keep query focused on the original clinical question
    ...
    def _scan_quantitative(synthesis: str) -> dict:
        ...
        if not synthesis:
            return {"pct": 0, "hr": 0, "pval": 0, "mol": 0}
        ...
        return {"pct": pct, "hr": hr, "pval": pval, "mol": mol}
    ...
    async def _check_trial_sufficiency(self, trial_name: str, sub_queries: list[str], synthesis: str) -> dict:
        ...
        if fact_parts:
            fact_line = "正则扫描已确认：合成文本中客观存在 " + "、".join(fact_parts) + "。这些数据点不容否认。"
        else:
            fact_line = "正则扫描未检测到任何定量数据（无百分比、HR、P值、分子分型词）。"
        ...
        prompt = textwrap.dedent(f"""
        你是一个循证医学数据审核员。请阅读以下 **{trial_name}** 的完整合成文本，判断其是否充分覆盖了临床试验的核心数据维度。

        【⚠️ 事实核查前置——正则扫描已确认】：{fact_line}

        【你的任务——在完整语义上下文中评估】：
        阅读下方完整合成文本，判断三个维度是否均有**临床有意义的真实数值**（排除模板占位符如"[N=具体数字]"）：
        1. 生存数据：是否包含 OS/PFS/RFS 的具体百分比或比较结果？
        2. 统计学指标：是否包含 HR 或 P 值？
        3. 分子亚组：是否提及分子分型（p53/TP53、MMR/MSI、POLE）相关数据？

        【判定——在充分理解上下文后判断】：
        - 三项均有真实数值 → sufficient=true
        - 正则已扫到数据但你确认仅为占位符/模板残留 → 可在 reason 中说明为何判定为无效
        - 某项确实缺失 → sufficient=false，refined_query 中针对该维度补充统计缩写或分子特征词

        【补充检索词规则】：
        - 🚨 **必须保留试验名称**：{trial_name} 必须作为 AND 的第一个条件（如 `{trial_name} AND ...`），绝对禁止生成不含试验名的泛化检索词
        - 保留疾病上下文：{disease_context if disease_context else 'endometrial cancer'}
        - 缺生存数据 → 加入 (OS OR PFS OR RFS OR HR)
        - 缺分子数据 → 加入 (p53 OR TP53 OR POLE OR MMR OR MSI)
        - 禁止添加患者合并症术语
    ...
    async def _synthesize(self, query: str, results: list, ref_map: str) -> str:
        ...
        prompt = textwrap.dedent(f"""
        你是一名循证医学分析专家。请根据以下检索结果，提炼与检索问题相关的临床证据。
        ...
        【核心防伪与质量规则——必须逐条遵守】：
        1. 样本量绝对防伪：样本量（N）必须绝对提取自原文文本！若原文摘要未提及数字，只能写"未报告"。绝对禁止使用"1000多例"、"约数百例"等任何推测性或概括性的虚假词汇！
        2. 严格对照组归因法则：CRT vs RT 只能谈化疗获益；CRT vs CT 只能谈放疗获益，禁止错乱归因。
        3. 数值对位安全核查：在提取HR、OS、PFS等数值时，必须严格确保指标名称与数值一一对应，绝对禁止顺序错位！

        【输出格式】每篇文献必须像"做填空题"一样，严格复制以下 Markdown 模板。前缀文字（如 `- 试验组（或观察队列1）：`）必须一字不差地输出，绝对禁止修改前缀名称！绝对禁止删除任何一行！

        #### [填入原文英文标题] [^^n]

        [首段：用1-2句中文自然描述试验设计、样本量、纳排标准。若原文无样本量，在此处写"样本量未报告"。]

        - 试验组（或观察队列1）：[填入具体方案内容。若原文为纯观察性/标志物分析无干预对比，必须填写："本研究为预后/分子特征分析，无特定干预分组"]
        - 对照组（或观察队列2）：[填入具体方案内容。若无对照组，必须填写："无"]

        • [疗效数据点1——填入中文描述 + 具体数值（% / HR / 95%CI / P值）。注意必须用 `• ` 符号开头！]
        • [疗效数据点2——同上]
        • [结论/临床解读——1-2句概括核心发现。]
        """)
    ...
    async def _generate_relaxed_query(self, original_query: str) -> str:
        ...
        """
        prompt = textwrap.dedent(f"""
        你是一名PubMed检索策略专家。以下检索词没有搜到任何结果，请生成一个更宽松的版本。

        【原始检索词】：{original_query}

        【宽松策略】：
        1. **减少AND条件**：砍掉次要维度，最多保留2个核心维度用AND连接
        2. **展开同义词**：对核心概念用OR连接更多等效表述
        3. **移除字段限制**：去掉 [Title/Abstract] 等字段限定
        4. **保留年份**：保留 2018:2026[dp] 即可

        【输出要求】：
        直接输出宽松后的检索词，不要带任何解释、引号或代码块标记。确保是一个可粘贴到PubMed中的检索字符串。
        """)
