...
from ...utilities.patient_state import classify_surgery, build_surgery_anatomy_rules
...
class NCCNFollowupSkill:
    ...
    async def generate(self, report_model, treatment_context: str, surgery_type: str = "",
        ...
        4. **覆盖六个方面**：随访频率、手术并发症急诊指征、警示症状、随访检查内容、患者教育、心理社会支持。
        5. **合并症管理不属于随访方案**：合并症管理已在术后处理章节完整覆盖，随访方案中不再重复列出。
        6. **[^^n] 角标**：患者临床信息中如果带有 [^^n] 角标，原样保留。
        ...
        严格按以下 6 条格式输出，每条一个自然段，不得拆分：

        1、按照[根据NCCN指南确定的风险分层]随访。随访频率：术后 2 年每 3 个月随访 1 次，以后每 6 个月随访 1 次直至第 5 年，之后每年一次。
        ...
        - **禁止**使用 ### 小标题拆分内容
        - **禁止**使用表格
        - **禁止**将单条拆分为多个缩进子项或编号子列表
        - **禁止**在 6 条之外增加任何条目（包括合并症随诊、科室转诊等）
        - 输出必须是连续编号 1-6 的自然段，每段一段话

        🔴 **【中文输出——全字段强制要求】**：所有描述性文字必须使用中文，数值和统计量保留原文。**绝对禁止**直接复制粘贴英文段落。

        🚨 **【受众定位——强制约束】**：这份报告的读者是**管床医生和 MDT 会诊专家**，不是患者本人。全程使用临床第三人称客观陈述（"患者应..."、"建议患者..."），**绝对禁止**使用第二人称（"您"/"你"）。

        你的输出将直接呈现给管床医生和 MDT 会诊专家。**严禁**在最终输出中出现：元指令标签、对模型下达命令的句式、条件判断句式、第二人称"您""你"。输出序号必须从 1 开始连续编号。违反以上任何一条，输出将被视为不合格。
        ...
        for attempt in range(2):
            try:
                ...
                result = depersonalize_report(result)
                # 后处理：JSON → 自然文本
