...
bounded_gather 创建的 asyncio.Task 自动注册到 ShutdownManager。
"""
...
async def bounded_gather(*coros, concurrency: int = 10,
    ...
    """限制并发的 asyncio.gather，自动注册任务到 ShutdownManager。
    ...
    if return_exceptions:
        return results
    # 非 return_exceptions 模式：重新抛出第一个异常
    for item in results:
