...
def create_tool_excel():
    ...
    # 自动查找路径
    base_dirs = [Path("local_deep_research"), Path(".")]
    target_dir = None
    for d in base_dirs:
    ...
    if not target_dir:
        # 如果没找到，默认创建在 local_deep_research 下
        target_dir = Path("local_deep_research/cache_data")
...
if __name__ == "__main__":
    ...
    if create_tool_excel():
        ...
        print("\n请运行以下命令启动审计:")
        print("  python local_deep_research/main.py --mode audit --context_file your_plan.txt")